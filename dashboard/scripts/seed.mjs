...
const dbPath = path.resolve('marketing-os.db');
const db = new Database(dbPath);

console.log('Seeding clientes Marketing OS em: ' + dbPath);
...
const clientes = [
  { id: 'c01', nome: 'cliente-01', segmento: 'Varejo', fase_atual: 3, bloqueio_midia: 0 },
  { id: 'c02', nome: 'FitLife Studio', segmento: 'Fitness', fase_atual: 1, bloqueio_midia: 0 },
  { id: 'c03', nome: 'Megahair', segmento: 'Beleza', fase_atual: 1, bloqueio_midia: 1 },
  { id: 'c04', nome: 'Noctua', segmento: 'Segurança', fase_atual: 2, bloqueio_midia: 0 }
];

const stmtCliente = db.prepare('INSERT OR IGNORE INTO clientes (id, nome, segmento, fase_atual, bloqueio_midia) VALUES (?, ?, ?, ?, ?)');
const stmtArtefato = db.prepare('INSERT OR IGNORE INTO artefatos (id, cliente_id, tipo, status) VALUES (?, ?, ?, ?)');

for (const c of clientes) {
  try {
    stmtCliente.run(c.id, c.nome, c.segmento, c.fase_atual, c.bloqueio_midia);
    
    // Artefatos padrão para todos
    ...
    console.error("Erro ao inserir " + c.nome + ":", e.message);
  }
}
